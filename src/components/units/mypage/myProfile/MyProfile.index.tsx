						...
						<S.SignBox>
							<S.InputBox style={{ height: '600px' }}>
								...
								<S.InputCommon
									...
									{...infoRegister('name')}
								/>
								<S.ErrorCommon>{infoFormState.errors.name?.message}</S.ErrorCommon>
								<S.Label>사진</S.Label>
								{fileUrl !== '' ? (
									<S.ImgBox>
										<S.UploadImage onClick={onClickUpload} src={`${fileUrl}`} />
										<S.CloseBtn src="/icon/close.svg" onClick={onClickDeleteUrl} />
									</S.ImgBox>
								) : (
									<S.ImgBox>
								...
								/>
								<S.ErrorCommon>{infoFormState.errors.image?.message}</S.ErrorCommon>
							</S.InputBox>
